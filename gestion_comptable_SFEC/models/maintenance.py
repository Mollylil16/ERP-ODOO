...
class Maintenance(models.AbstractModel):
    _name = 'custom_accounting.maintenance'
    ...
    def cleanup_old_commands(self, days=60):
        ...
        old_commands = self.env['custom_accounting.bon_commande_client'].search([
            ('state', '=', 'cancelled'),
    ...
    def cleanup_old_invoices(self, days=90):
        ...
        threshold_date = datetime.now() - timedelta(days=days)
        old_invoices = self.env['custom_accounting.facture_finale'].search([
            ('state', '=', 'cancelled'),
    ...
    def cleanup_old_payments(self, days=180):
        ...
        threshold_date = datetime.now() - timedelta(days=days)
        old_payments = self.env['custom_accounting.paiement_client'].search([
            ('state', '=', 'cancelled'),
    ...
    def check_stock_levels(self):
        ...
        low_stock_products = self.env['custom_accounting.stock_mouvement'].search([
            ('stock_actuel', '<', 10),
        ...
        if low_stock_products:
            for product in low_stock_products:
                self.env['mail.message'].create({
                    'model': 'custom_accounting.stock_mouvement',
                    'res_id': product.id,
    ...
    def check_constraints(self):
        ...
        models = ['custom_accounting.bon_commande_client',
                 'custom_accounting.facture_finale',
                 'custom_accounting.paiement_client']
    ...
    def check_emails(self):
        ...
        models = ['custom_accounting.client', 'custom_accounting.fournisseur']
