...
class ReceptionFournisseur(models.Model):
    _name = 'custom_accounting.reception_fournisseur'
    _description = "Réception de commande fournisseur"

    bon_commande_id = fields.Many2one('custom_accounting.bon_commande_fournisseur', string="Bon de commande", required=True)
    date_reception = fields.Date(string="Date de réception", default=date.today)
    notes = fields.Text(string="Remarques")
    reception_ligne_ids = fields.One2many('custom_accounting.ligne_reception_fournisseur', 'reception_id', string="Articles réceptionnés")
