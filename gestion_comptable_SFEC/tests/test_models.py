...
class TestModels(common.TransactionCase):
    def setUp(self):
        super().setUp()
        self.client = self.env['custom_accounting.client']
        self.fournisseur = self.env['custom_accounting.fournisseur']
        self.commande = self.env['custom_accounting.bon_commande_client']
        self.facture = self.env['custom_accounting.facture_finale']
        self.paiement = self.env['custom_accounting.paiement_client']
    ...
    def test_client_creation(self):
        ...
        client = self.client.create({
            ...
            'entreprise_id': self.env['custom_accounting.entreprise'].create({'name': 'Test Entreprise'}).id
        })
    ...
    def test_commande_workflow(self):
        ...
        client = self.env['custom_accounting.client'].create({
            'name': 'Test Client',
            'email': 'test@example.com'
        })
    ...
    def test_constraint_validation(self):
        ...
        with self.assertRaises(ValidationError):
            ...
            self.commande.create({
                'name': 'BC-001',
                'client_id': self.env['custom_accounting.client'].create({'name': 'Test Client'}).id
            })
            self.commande.create({
                'name': 'BC-001',
                'client_id': self.env['custom_accounting.client'].create({'name': 'Test Client'}).id
